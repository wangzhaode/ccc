...
Agent Loop 是 ccc 的核心引擎，实现了"用户输入 → LLM 推理 → 工具执行 → 继续推理"的循环。API Client 负责与 LLM 提供商通信，支持 Anthropic 和 OpenAI 两种协议。
...
`ApiClient` 类从配置文件 `~/.ccc/settings.json` 读取 API 配置，自动初始化对应的 API 提供商：
...
配置优先级：`~/.ccc/settings.json` > 环境变量 > 默认值。
...
通过 `memory_manager_.build_memory_prompt()` 注入 CCC.md 的内容。如果找到了 CCC.md 文件，其内容会被添加到 `# Project Instructions` 部分。
