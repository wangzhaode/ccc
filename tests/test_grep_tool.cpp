 ...
 protected:
  ...
  void SetUp() override {
    ...
    write_file(test_dir + "/src/util.cpp",
               "#include \"util.h\"\nvoid helper() {\n    // TODO: implement\n}\n");
    write_file(test_dir + "/readme.txt", "This is a readme file.\nIt has multiple lines.\nEnd.\n");
  }
...
TEST_F(GrepToolTest, FindPattern) {
  auto result = tool.execute({{"pattern", "include"}, {"path", test_dir}});
...
TEST_F(GrepToolTest, ContentMode) {
  auto result = tool.execute({{"pattern", "TODO"}, {"path", test_dir}, {"output_mode", "content"}});
  EXPECT_FALSE(result.is_error);
...
TEST_F(GrepToolTest, GlobFilter) {
  auto result = tool.execute({{"pattern", "include"}, {"path", test_dir}, {"glob", "**/*.cpp"}});
  EXPECT_FALSE(result.is_error);
  EXPECT_NE(result.content.find("main.cpp"), std::string::npos);
...
TEST_F(GrepToolTest, NoMatches) {
  auto result = tool.execute({{"pattern", "xyz_not_here"}, {"path", test_dir}});
  EXPECT_FALSE(result.is_error);
  EXPECT_NE(result.content.find("No matches"), std::string::npos);
}

TEST_F(GrepToolTest, InvalidRegex) {
  auto result = tool.execute({{"pattern", "[invalid"}, {"path", test_dir}});
...
TEST_F(GrepToolTest, SingleFile) {
  auto result = tool.execute(
      {{"pattern", "hello"}, {"path", test_dir + "/src/main.cpp"}, {"output_mode", "content"}});
  EXPECT_FALSE(result.is_error);
