...
int ContextManager::estimate_total_tokens(const json& system_prompt,
                                           const json& messages,
...
bool ContextManager::maybe_compress(json& messages, const json& system_prompt,
...
bool ContextManager::force_compress(json& messages, const json& system_prompt,
                                     const json& tools, ApiClient& api) {
...
bool ContextManager::compress(json& messages, ApiClient& api) {
    ...
    summary_system.push_back({
        {"type", "text"},
        {"text", "You are a conversation summarizer. Summarize the following conversation history concisely, "
    ...
    try {
        ApiResponse summary_response = api.chat(summary_system, summary_messages,
                                                 json::array(), nullptr);
        ...
        ui::print_warning("Context compressed: " + std::to_string(old_count) +
                          " messages summarized.");
        return true;
