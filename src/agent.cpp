...
# Environment
 - Working directory: )" +
                std::filesystem::current_path().string() + R"(
 - Platform: )"
...
json Agent::build_user_message(const std::string& user_input) const {
  ...
  // User's actual text
  content.push_back({{"type", "text"}, {"text", user_input}});
...
json Agent::execute_tool_call(const json& tool_use_block) {
  ...
  auto make_error = [&](const std::string& msg) -> json {
    ...
    return {
        {"type", "tool_result"}, {"tool_use_id", tool_id}, {"content", msg}, {"is_error", true}};
  };
...
void Agent::agent_loop() {
  ...
  for (int iter = 0; iter < max_iterations; iter++) {
    ...
    try {
      response =
          api_client_.chat(system_prompt, messages_, tools,
    ...
    snprintf(stats_buf, sizeof(stats_buf),
             "(%.1fs \xc2\xb7 \xe2\x86\x91 %d tokens \xc2\xb7 \xe2\x86\x93 %d tokens \xc2\xb7 "
             "thought for %.1fs)",
             total_duration.count(), response.input_tokens, response.output_tokens,
             thought_duration.count());
