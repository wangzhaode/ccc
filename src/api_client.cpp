...
ApiClient::ApiClient() {
  ...
  std::string provider_str = settings.value("provider", get_env("API_PROVIDER", "anthropic"));
  ...
  if (api_key_.empty()) {
    std::cerr << "Warning: API key not set. Configure ~/.ccc/settings.json or set environment "
                 "variables.\n";
  }
}
...
std::string ApiClient::system_to_string(const json& system_prompt) {
  if (system_prompt.is_string())
    return system_prompt.get<std::string>();
...
ApiResponse ApiClient::chat(const json& system_prompt, const json& messages, const json& tools,
...
static void parse_sse_lines(const std::string& body, std::function<void(const json&)> on_event) {
...
ApiResponse ApiClient::chat_anthropic(const json& system_prompt, const json& messages,
                                      const json& tools, StreamTextCallback on_text) {
  json body;
  body["model"] = model_;
  ...
  httplib::Headers headers = {{"x-api-key", api_key_}, {"anthropic-version", "2023-06-01"}};
  ...
  if (!res || res->status != 200) {
    ...
    throw std::runtime_error("Anthropic API error (HTTP " + std::to_string(status) +
                             "): " + error_msg);
  }
    ...
    if (type == "message_start") {
      ...
      if (delta_type == "text_delta") {
        ...
        if (it != index_to_block.end() && it->second < (int)content_blocks.size()) {
          content_blocks[it->second]["text"] =
              content_blocks[it->second]["text"].get<std::string>() + text;
        }
        if (on_text)
          on_text(text);
  ...
  result.message = {{"role", "assistant"}, {"content", content_blocks}};
  ...
  for (auto& block : msg["content"]) {
    if (block["type"] == "text") {
      ...
      tool_calls_arr.push_back(
          {{"id", block["id"]},
           {"type", "function"},
           {"function", {{"name", block["name"]}, {"arguments", block["input"].dump()}}}});
    }
  }
...
ApiResponse ApiClient::chat_openai(const json& system_prompt, const json& messages,
                                   const json& tools, StreamTextCallback on_text) {
  ...
  if (!res || (res->status != 200)) {
    std::string error_msg = res ? res->body : "Connection failed";
    int status = res ? res->status : 0;
    throw std::runtime_error("OpenAI API error (HTTP " + std::to_string(status) +
                             "): " + error_msg);
  }
...
json ApiClient::tools_to_openai(const json& tools) {
  ...
  for (auto& tool : tools) {
    oai_tools.push_back({{"type", "function"},
                         {"function",
