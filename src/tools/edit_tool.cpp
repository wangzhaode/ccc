...
ToolResult EditTool::execute(const json& params) {
  ...
  if (count > 1 && !replace_all) {
    return {"Error: old_string found " + std::to_string(count) +
                " times. Provide more context to make it unique, or set replace_all to true.",
            true};
  }
  ...
  std::string msg = "Edited " + file_path + ": replaced " +
                    std::to_string(replace_all ? count : 1) + " occurrence(s).";
  return {msg, false};
}
